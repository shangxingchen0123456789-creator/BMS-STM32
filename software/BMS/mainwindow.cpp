    ...
    , host(new BmsHost(this))
{
    ...
    connect(host, &BmsHost::ackTimeout, this, [this](quint8 seq) {
        onLog(QString("[WARN] ACK timeout: seq=%1, check BMS connection").arg(seq));
        statusBar()->showMessage(QString("ACK timeout (seq=%1)").arg(seq), 5000);
    });
    // 初始界面
